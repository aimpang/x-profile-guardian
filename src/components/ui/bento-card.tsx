...
const BentoCard = () => {
  ...
  const content = useMemo(() => {
    switch (activeTab.id) {
      ...
      case "threads": return <ThreadsDashboard />;
      case "resources": return <ResourcesDashboard />;
      default: return null;
    }
  }, [activeTab.id]);
  ...
  return (
    <div className="flex items-center justify-center w-full antialiased">
      <div className="group relative w-full max-w-xl overflow-hidden rounded-3xl border bg-card shadow-2xl shadow-primary/5 transition-all duration-500 hover:shadow-primary/10 hover:-translate-y-1 m-0">
        ...
        <div className="relative w-full h-[260px] sm:h-[300px] overflow-hidden rounded-2xl">
  ...
  <div className="flex flex-col gap-3 h-full">
      ...
      <div className="flex flex-col gap-2 relative z-10">
        ...
        <div className="flex flex-col gap-0.5">
          ...
          <div className="w-full h-1 bg-muted rounded-full overflow-hidden mt-1">
            <motion.div initial={{ width: 0 }} animate={{ width: "94.2%" }} className="h-full bg-primary rounded-full" />
          </div>
        </div>
    ...
    <div className="grid grid-cols-2 gap-2">
      {[
        ...
        <div key={i} className="p-3 rounded-xl border border-border/40 bg-background/50 flex items-center justify-between">
          <div className="flex flex-col">
            <span className="text-[10px] font-medium text-foreground">{item.value}</span>
            <span className="text-[8px] text-muted-foreground uppercase font-medium">{item.label}</span>
          </div>
          <HugeiconsIcon icon={item.icon} size={14} className="opacity-20" />
        </div>
      ))}
    </div>
  </div>
    ...
    <div className="grid grid-cols-2 gap-3">
      {[
        ...
        <div key={i} className="p-3.5 rounded-xl border border-border/40 bg-background/50 flex flex-col gap-3 relative overflow-hidden group">
          ...
          <button className="w-fit flex items-center gap-1.5 px-2 py-1 rounded-md bg-foreground text-background text-[8px] font-semibold group-hover:bg-primary z-10">
            <HugeiconsIcon icon={Add01Icon} size={8} strokeWidth={3} /> Create
          </button>
        </div>
      ))}
    </div>
